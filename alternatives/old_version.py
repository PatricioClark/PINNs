...
class PhysicsInformedNN:
    ...
    def __init__(self,
        ...
        # Hidden layers
        hid_layers = [keras.layers.Dense(width, activation=self.activation)
                      for _ in range(depth)]

        # Feature expansion
        # Estaría bueno guardar las B's
        rnd_seed = 1234
        std_devs = [1, 2, 4, 8, 16]
        rng = np.random.default_rng(rnd_seed)
        feat_out = []
        for sigma in std_devs:
            B = rng.normal(0, sigma, (self.din, width))
            xx = tf.linalg.matmul(coords, B)
            gamma = tf.concat([tf.math.cos(2.0*np.pi*xx),
                               tf.math.sin(2.0*np.pi*xx)], 1)
            for hid in hid_layers:
                gamma = hid(gamma)
            feat_out.append(gamma)

        # Output definition
        fields = keras.layers.Dense(self.dout, name='fields')(feat_out)
