...
def generate_data(Nt, Nx, Ny, Nz, sample_prob):
    ...
    dx_dns   = 8*np.pi/2048
    dx       = 8*np.pi/2048*(bound_x//2)
    dz       = 3*np.pi/1536*(bound_z//2)
    y0       = 250
    y_points = np.loadtxt('y_points.txt')[y0:y0+Ny]
    t_d, x_d, y_d, z_d = [], [], [], []
    u_d, v_d, w_d      = [], [], []
    for tidx in range(Nt):
        velocity = np.load('data/filt_velos.{:02}.npy'.format(tidx))
        i0 = round(0.45*dt*tidx/dx_dns)
        for pp in points:
            ix, iy, iz = pp
            t_d.append(tidx*dt)
            x_d.append(ix*dx + 0.45*tidx*dt - i0*dx_dns)
