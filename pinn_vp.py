...
class PhysicsInformedNN:
    ...
    def training_step(self, X_batch, Y_batch,
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            Y_pred, p_pred = self.curl(X_batch)
    ...
    @tf.function
    def curl(self, coords):
        with tf.GradientTape(persistent=True) as tape:
            tape.watch(coords)
            output = self.model(coords, training=True)
            Y_pred = output[0]
            p_pred = output[1:]
            u  = Y_pred[:,0] 
            v  = Y_pred[:,1] 
        ...
        grad_v = tape.gradient(v, coords)
        v_t = grad_v[:,0:1]
        v_x = grad_v[:,1:2]
        v_y = grad_v[:,2:3]
        v_z = grad_v[:,3:4]

        grad_w = tape.gradient(w, coords)
        w_t = grad_w[:,0:1]
        w_x = grad_w[:,1:2]
        w_y = grad_w[:,2:3]
        w_z = grad_w[:,3:4]
        ...
        # Normalize
        # u = self.sg[0]*u + self.mm[0]
        # v = self.sg[1]*v + self.mm[1]
        # w = self.sg[2]*w + self.mm[2]
        # p = self.sg[3]*p + self.mm[3]

        # Concatenate
        div_free_sol = tf.concat([u,v,w,p], 1)

        return [div_free_sol, p_pred]
